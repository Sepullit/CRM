(()=>{
  ...
  // Информация о контактах и тултипы
  ...
  function createContactsInfo(contacts) {
    ...
    contacts.forEach((contact, index)=> {
      ...
      if(index >= 4) {
        const tooltipOther = document.createElement('span')
        tooltipOther.textContent = 'Показать все контакты'
        tooltipOther.classList.add('contacts__item-tooltip')
        item.classList.add('display-none')
        itemNum.classList.add('contacts__item-sum')
        itemNum.textContent = `+${contacts.length - maxItem}`
        itemNum.append(tooltipOther)
      }
  ...
  function showsClients(data) {
    ...
    if(data.length > 0) {
      data.forEach((element) => {
        ...
        tdUserContacts.append(createContactsInfo(element.contacts))
  ...
  function createTable() {
    ...
    function createTableBody() {
      ...
      tbodyContainer.append(circleLoader)
  ...
  // Функции сортировки элементов в таблице
...
//Поиск клиента
  ...
  async function requestSearchClients(request) {
  ...
  async function requestSearchClientsForId(id) {
  ...
  function createModal(typeModal, user) {
    ...
    body.append(modal)
    body.classList.add('modal__open')
    if(typeModal !== 'deleteClient' ) activateDisableButton()
    return modal
  }
  ...
  function modalFooter(typeModal, user) {
    ...
    mainButton.addEventListener('click', async (e) => {
      e.preventDefault()
      if(typeModal === 'deleteClient') {
        const response = await deleteClient(user)
        ...
        if(validationForm()) {
          ...
          if(typeModal === 'newUser') {
            const response = await addClient(value)
            const errorRespone = catchResponse(response, error)
            data = await getData()
          ...
          if(typeModal === 'changeClient') {
            const response = await editClient(value, user.id)
            const errorRespone = catchResponse(response, error)
            data = await getData()
...
// Функции добавления/удаления/изменения клиентов
  ...
  async function addClient(objUser) {
    const response = await fetch(mainURL, {
      method: "POST",
  ...
  async function editClient(objUser, id) {
    const response = await fetch(`${mainURL}/${id}`, {
      method: "PATCH",
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(objUser),
  ...
  async function runApp() {
    ...
    tableContainer.append(createTitle('Клиенты', 'main__title'))
    ...
    container.append(buttonAddClient('Добавить пользователя'))
    ...
    input.addEventListener('input', () => {
      ...
      if(value) {
        ...
        timerInput = setTimeout(async () => {
          const data = await requestSearchClients(value)
          showsClients(data)
    ...
    if(!!hash) {
      const user = await requestSearchClientsForId(hash)
